...
class lucida_metabox {
	...
	*/
	public function show() {
		...
		$sidebar_options		= lucida_metabox_sidebar_options();
	    ...
	    <div id="lucida-ui-tabs" class="ui-tabs">
		    <ul class="lucida-ui-tabs-nav" id="lucida-ui-tabs-nav">
		    	<li><a href="#frag1"><?php esc_html_e( 'Layout Options', 'lucida' ); ?></a></li>
		    	<li><a href="#frag3"><?php esc_html_e( 'Header Featured Image Options', 'lucida' ); ?></a></li>
		    	<li><a href="#frag4"><?php esc_html_e( 'Single Page/Post Image Layout ', 'lucida' ); ?></a></li>
		    </ul>
		    ...
		    <div id="frag1" class="catch_ad_tabhead">
		    	<table id="layout-options" class="form-table" width="100%">
		            <tbody>
		                <tr>
		                    <select name="lucida-layout-option" id="custom_element_grid_class">
		      					<?php
			                    foreach ( $layout_options as $field ) {
			                        $metalayout = get_post_meta( $post->ID, 'lucida-layout-option', true );
			                        if ( empty( $metalayout ) ){
			                            $metalayout='default';
			                        }
			                   	?>
		                    ...
		                    <select name="clean-business-header-image" id="custom_element_grid_class">
			                     <?php
				                    foreach ( $featured_image_options as $field ) {
				                        $metalayout = get_post_meta( $post->ID, 'lucida-header-image', true );
				                        if ( empty( $metaimage ) ){
				                            $metaimage='default';
				                        }
				                   	?>
...
$lucida_metabox = new lucida_metabox(
	...
	__( 'Lucida Options', 'lucida' ), //metabox title
