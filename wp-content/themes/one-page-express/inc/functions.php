...
function one_page_express_print_header_image()
{
    ...
    if (!empty($image)) {
        printf('<img src="%1$s"/>', esc_url($image));
    }
}
...
function one_page_express_print_header_title()
{
    ...
    if ($show && !empty($title)) {
        printf('<h1 %1$s class="heading8">%2$s</h1>', $text_effect_attr, wp_kses_post($title));
    }
}
...
function one_page_express_print_header_subtitle()
{
    ...
    if ($show && !empty($subtitle)) {
        printf('<p %1$s class="header-subtitle">%2$s</p>', $text_effect_attr, wp_kses_post($subtitle));
    }
}
...
function one_page_express_print_header_button_1()
{
    ...
    if ($show && $title) {
        printf('<a class="button blue big" href="%1$s">%2$s</a>', esc_url($url), wp_kses_post($title));
    }
}
...
function one_page_express_print_header_button_2()
{
    ...
    if ($show && $title) {
        printf('<a class="button green big" href="%1$s">%2$s</a>', esc_url($url), wp_kses_post($title));
    }
}
...
function one_page_express_add_sections($wp_customize)
{
    ...
    $wp_customize->add_section('one_page_express_footer_template', array(
        'title' => __('Footer', 'one-page-express'),
        'priority'       => 3,
    ));
    ...
    $sections = array(
        'one_page_express_header_layout' => array(
            'title' => __('Front Page Header Presets', 'one-page-express'),
            'priority'       => 1,
        ),
...
function one_page_express_header_presets()
{
    ...
    if(isset($ONE_PAGE_EXPRESS_PRESETS))   {
        return $ONE_PAGE_EXPRESS_PRESETS;
    }
    ...
    $wp_customize->add_control(new OnePageExpress\BackgroundTypesControl($wp_customize, $prefix.'_background_type', array(
        ...
        "choices"               => array(
           "image"              => array(
               ...
               "control"        => $inner ? "header_image": "one_page_express_header_image"
            ),
            "gradient"          => array(
                "label"         => __("Gradient", "one-page-express"),
                "control"       => $prefix."_gradient",
            ),
            "slideshow"         => array(
...
function one_page_express_hero_data()
{
    return array(
        'title' => get_theme_mod('one_page_express_header_title', __('STAND OUT FROM THE CROWD!', 'one-page-express')),
        'subtitle' => get_theme_mod('one_page_express_header_title', __('OnePage Express supports a custom frontpage', 'one-page-express')),
        'image' => get_theme_mod('one_page_express_header_content_image', get_template_directory_uri() . "/screenshot.jpg"),
        'btn_1_url' => get_theme_mod('one_page_express_header_btn_1_url', '#'),
        'btn_1_title' => get_theme_mod('one_page_express_header_btn_1_label', __('GET STARTED TODAY', 'one-page-express')),
        'btn_2_url' => get_theme_mod('one_page_express_header_btn_2_url', '#'),
        'btn_2_title' => get_theme_mod('one_page_express_header_btn_2_label', __('Learn More', 'one-page-express')),
    );
}
...
{
    wp_enqueue_style('one-page-express-fonts', '//fonts.googleapis.com/css?family=Lato:300,400,700,900|Open+Sans:400,300,600,700|Source+Sans+Pro:200,normal,300,600,700');
    ...
    wp_enqueue_script('one-page-express-morphext', get_template_directory_uri() . '/assets/js/libs/morphext.js', array('jquery'), $ver, true);
    ...
    $one_page_express_jssettings = array(
        'header_text_morph_animation' => get_theme_mod('one_page_express_header_text_morph_animation', 'flipInX'),
        'header_text_morph_speed'     => intval(get_theme_mod('one_page_express_header_text_morph_speed', 2000)),
        'header_text_morph'           => get_theme_mod('one_page_express_header_show_text_morph_animation', true)
    );

    wp_localize_script('jquery', 'one_page_express_settings', $one_page_express_jssettings);

    $maxheight = get_theme_mod('one_page_express_logo_max_height', 70);
    wp_add_inline_style('one-page-express-style',  sprintf('img.custom-logo{max-height:%1$s;}', $maxheight."px"));



    $transparent_nav = get_theme_mod($prefix. '_nav_transparent', true);
    $sticked_nav_color = "transparent";
   
    if (!$transparent_nav) {
        $sticked_nav_color = "#ffffff";
    }

    wp_add_inline_style('one-page-express-style',  sprintf('.header-top{background-color:%1$s;}', $sticked_nav_color));
...
{
    return '&copy;&nbsp;' . "&nbsp;" . date('Y') . '&nbsp;' . esc_html(get_bloginfo('name')) . '.&nbsp;' . __('Built using WordPress and OnePage Express Theme.', 'one-page-express');
}
...
function one_page_express_title()
{
    ...
    if (is_404()) {
        $title['title'] = __('Page not found', 'one-page-express');
    } elseif (is_search()) {
        $title['title'] = sprintf(__('Search Results for &#8220;%s&#8221;', 'one-page-express'), get_search_query());
    } elseif (is_home()) {
        if (is_front_page()) {
...
function one_page_express_pagination($args = [], $class = 'pagination')
{
...
add_action('wp_head', function () {
    ...
    if (true == get_theme_mod('one_page_express_blog_header_overlap',true)): ?>
